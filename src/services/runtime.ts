...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.worldmonitor.app') || host.endsWith('.cooperation.tw');
  } catch {
    return false;
  }
}
...
const ALLOWED_REDIRECT_HOSTS = /^https:\/\/([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)*(?:worldmonitor\.app|cooperation\.tw)(:\d+)?$/;
